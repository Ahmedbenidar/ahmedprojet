...
public function __construct($firstname,$lastname,$email,$password,$idCity,$idfiliere){
...
$sql = "INSERT INTO $tableName  (firstname, lastname, email,password,idCity,idfiliere)
VALUES ('$this->firstname', '$this->lastname', '$this->email','$this->password','$this->idCity','$this->idfiliere')";
...
$sql = "SELECT id, firstname, lastname, email, idCity FROM $tableName,idfiliere FROM $tableName";
...
header('Location:read.php');
    ...
    public static function  selectClientsByCity($tableName,$conn,$idCity){
        ...
        //select all the client from database, and inset the rows results in an array $data[]
        $sql = "SELECT id, firstname, lastname, email, idCity FROM $tableName WHERE idCity = '$idCity',idfiliere FROM $tableName WHERE idfiliere = '$idfiliere'";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) {
        // output data of each row
        $table=[];
